...
export const registerservice_provider = async (req, res) => {
  ...
  try {
    ...
    const createduser = await Service_Provider.findById(
      nserviceprovider._id
    ).select("-SellerPassword");
    if (!createduser) {
      return res
        .status(400)
        .json({
          message: "User not created, something went wrong while registering",
        });
    }
...
const generateToken = (userId) => {
  const accessToken = jwt.sign(
    { userId },
    process.env.ACCESS_TOKEN_SECURITY_KEY,
...
export const loginseller = async (req, res) => {
  ...
  const loggedinuser = await Service_Provider.findById(user._id).select(
    "-SellerPassword"
  );
